...
@property (nonatomic, strong) UIButton *ETCSignBtn;

@property (nonatomic, strong) UIButton *ERC20SignBtn;

@property (nonatomic, strong) UIButton *BTCSignBtn;
...
const uint32_t puiDerivePathBTC[] = {0, 0x8000002c, 0x80000000, 0x80000000, 0x00000000, 0x00000000};
const uint32_t puiDerivePathETC[] = {0, 0x8000002c, 0x8000003d, 0x80000000, 0x00000000, 0x00000000};
...
- (void)viewDidLoad {
    ...
    _walletCategoryList = [NSArray arrayWithObjects:_getAddressBtn, _getDeviceCheckCodeBtn, _ETHSignBtn, _EOSSignBtn, _CYBSignBtn,_signAbortBtn, _ETHSignNewBtn, _EOSSignNewBtn, _CYBSignNewBtn, _ETCSignBtn, _ERC20SignBtn, _BTCSignBtn, _SwitchSignBtn, _abortButton2Btn, nil];
...
//    }];
    ...
    [self.view addSubview:self.ETCSignBtn];
    [self.view addSubview:self.ERC20SignBtn];
    [self.view addSubview:self.BTCSignBtn];
    ...
    NSArray *cat4Arr4 = @[self.BTCSignBtn, self.ERC20SignBtn, self.ETCSignBtn];
    ...
    NSArray *cat4Arr5 = @[self.SwitchSignBtn, self.abortButton2Btn];
    [cat4Arr5 mas_distributeViewsAlongAxis:MASAxisTypeHorizontal withFixedSpacing:10 leadSpacing:30 tailSpacing:30];
    [cat4Arr5 mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setImageDataBtnAction
{
    ...
    NSError *error;
    NSString *path = [[[NSBundle mainBundle] resourcePath] stringByAppendingString:@"/logo"];
    if (![fileManager fileExistsAtPath:path]) {
        [self printLog:@"logo image not found"];
        return;
    }
    NSArray *imageNames = [fileManager contentsOfDirectoryAtPath:path error:&error];
    ...
    NSString *imagePath = [path stringByAppendingPathComponent:imageNames[imgIndex]];
    NSData *imageData = [fileManager contentsAtPath:imagePath];
    ...
    //According to Microsoft definition of BMP header:
    ...
    //        DWORD bfOffBits;
    //    } BITMAPFILEHEADER;
    //So the real data offset is BITMAPFILEHEADER.bfOffBits, 2+4+2+2 = 10 bytes off the start
    byte *dataPtr = (byte *)[imageData bytes];
    uint32_t offset = dataPtr[10] | dataPtr[11] << 8 | dataPtr[12] << 16 | dataPtr[13] << 24;
    int iRtn = PAEW_ConvertBMP(dataPtr + offset, imageData.length - offset, 128, 80, [destImg bytes], &destImgLen);
    if (iRtn != PAEW_RET_SUCCESS) {
...
- (void)getAddressBtnAction
{
    ...
    NSArray *coinNames = @[@"ETH", @"EOS", @"CYB", @"BTC", @"ETC"];
    ...
    switch (coinType) {
        ...
        case PAEW_COIN_TYPE_BTC:
            puiDerivePath = puiDerivePathBTC;
            derivePathLen = sizeof(puiDerivePathBTC)/sizeof(puiDerivePathBTC[0]);
            break;
        case PAEW_COIN_TYPE_ETC:
            puiDerivePath = puiDerivePathETC;
            derivePathLen = sizeof(puiDerivePathETC)/sizeof(puiDerivePathETC[0]);
            break;
        default:
            return;
    }
...
{
    if (!_BTCSignBtn) {
        _BTCSignBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_BTCSignBtn setTitle:@"BTCSign" forState:UIControlStateNormal];
        [_BTCSignBtn setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
        _BTCSignBtn.titleLabel.font = [UIFont systemFontOfSize:15.0 weight:UIFontWeightMedium];
        [_BTCSignBtn setBackgroundColor:[UIColor lightGrayColor]];
        [_BTCSignBtn addTarget:self action:@selector(BTCSignBtnAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _BTCSignBtn;
}

- (UIButton *)ERC20SignBtn
{
    if (!_ERC20SignBtn) {
        _ERC20SignBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_ERC20SignBtn setTitle:@"ERC20Sign" forState:UIControlStateNormal];
        [_ERC20SignBtn setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
        _ERC20SignBtn.titleLabel.font = [UIFont systemFontOfSize:15.0 weight:UIFontWeightMedium];
        [_ERC20SignBtn setBackgroundColor:[UIColor lightGrayColor]];
        [_ERC20SignBtn addTarget:self action:@selector(ERC20SignBtnAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _ERC20SignBtn;
}

- (UIButton *)ETCSignBtn
{
    if (!_ETCSignBtn) {
        _ETCSignBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_ETCSignBtn setTitle:@"ETCSign" forState:UIControlStateNormal];
        [_ETCSignBtn setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
        _ETCSignBtn.titleLabel.font = [UIFont systemFontOfSize:15.0 weight:UIFontWeightMedium];
        [_ETCSignBtn setBackgroundColor:[UIColor lightGrayColor]];
        [_ETCSignBtn addTarget:self action:@selector(ETCSignBtnAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _ETCSignBtn;
}
...
- (void)BTCSignBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        void *ppPAEWContext = (void*)self.savedDevice;
        int iRtn = PAEW_RET_UNKNOWN_FAIL;
        unsigned char nCoinType = PAEW_COIN_TYPE_BTC;
        [self printLog:@"ready for btc signature"];
        iRtn = PAEW_DeriveTradeAddress(ppPAEWContext, devIdx, nCoinType, puiDerivePathBTC, sizeof(puiDerivePathBTC)/sizeof(puiDerivePathBTC[0]));
        if (iRtn != PAEW_RET_SUCCESS) {
        ...
        const unsigned char    transaction[] = {  };
        ...
        iRtn = PAEW_BTC_SetTX(ppPAEWContext, devIdx, nUTXOCount, ppbUTXO, pnUTXOLen, transaction, sizeof(transaction));
        if (iRtn != PAEW_RET_SUCCESS) {
        ...
        while (YES) {
            ...
            iRtn = PAEW_BTC_GetSignResult(ppPAEWContext, devIdx, authType, 0, ppbTXSig[0], &(pnTXSigLen[0]));
            
            if (iRtn == PAEW_RET_SUCCESS) {
                [self printLog:@"BTC signature succeeded with signature0: %@", [Utils bytesToHexString:ppbTXSig[0] length:pnTXSigLen[0]]];
                //if GetSignResult returns success, then loop to get other signatures
                for (int i = 1; i < nUTXOCount; i++) {
                    iRtn = PAEW_BTC_GetSignResult(ppPAEWContext, devIdx, authType, i, ppbTXSig[i], &(pnTXSigLen[i]));
                    [self printLog:@"BTC signature succeeded with signature%d: %@", i,[Utils bytesToHexString:ppbTXSig[i] length:pnTXSigLen[i]]];
                }
                needAbort = false;
                break;
            } else if (lastResult != iRtn) {
                ...
                if (authType == PAEW_SIGN_AUTH_TYPE_FP) {
                    ...
                    if (lastResult != PAEW_RET_DEV_WAITING
                        ...
                        && lastResult != PAEW_RET_DEV_FP_NO_FINGER
                        && lastResult != PAEW_RET_DEV_FP_NOT_FULL_FINGER) {
                        [self printLog:@"BTC signature failed" ];
                        needAbort = true;
                        break;
                    }
                } else if (authType == PAEW_SIGN_AUTH_TYPE_PIN) {
...
- (void)ERC20SignBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        iRtn = PAEW_DeriveTradeAddress(ppPAEWContext, devIdx, nCoinType, puiDerivePathETH, sizeof(puiDerivePathETH)/sizeof(puiDerivePathETH[0]));
        if (iRtn != PAEW_RET_SUCCESS) {
            [self printLog:@"eth signature failed due to PAEW_DeriveTradeAddress returns : %@", [Utils errorCodeToString:iRtn]];
            return ;
        }
        
        unsigned char transaction[] = {0xf8, 0x69, 0x80, 0x84, 0xb2, 0xd0, 0x5e, 0x00, 0x83, 0x01, 0xd4, 0xc0, 0x94, 0x85, 0x9a, 0x9c, 0x0b, 0x44, 0xcb, 0x70, 0x66, 0xd9, 0x56, 0xa9, 0x58, 0xb0, 0xb8, 0x2e, 0x54, 0xc9, 0xe4, 0x4b, 0x4b, 0x80, 0xb8, 0x44, 0xa9, 0x05, 0x9c, 0xbb, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x32, 0x32, 0xeb, 0xb2, 0x32, 0x78, 0x6a, 0xff, 0x2d, 0xab, 0x33, 0xa6, 0xba, 0xdc, 0x17, 0x3a, 0x16, 0x56, 0xab, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1d, 0x24, 0xb2, 0xdf, 0xac, 0x52, 0x00, 0x00, 0x01, 0x80, 0x80};
        unsigned char pbTXSig[1024] = {0};
        size_t pnTXSigLen = 1024;
        BOOL pinVerified = NO;
        unsigned char authType = PAEW_SIGN_AUTH_TYPE_FP;
        
        iRtn = PAEW_SetERC20Info(ppPAEWContext, devIdx, PAEW_COIN_TYPE_ETH, "iETH", 18);
        ...
        while (YES) {
            ...
            if (iRtn == PAEW_RET_SUCCESS) {
                [self printLog:@"erc20 signature succeeded with signature: %@", [Utils bytesToHexString:pbTXSig length:pnTXSigLen]];
                needAbort = false;
                break;
            } else if (lastResult != iRtn) {
                [self printLog:@"%@ signature status : %@", (authType == PAEW_SIGN_AUTH_TYPE_FP ? @"Fingerprint" : @"PIN"), [Utils errorCodeToString:iRtn]];
...
- (void)ETCSignBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        void *ppPAEWContext = (void*)self.savedDevice;
        int iRtn = PAEW_RET_UNKNOWN_FAIL;
        unsigned char nCoinType = PAEW_COIN_TYPE_ETC;
        
        [self printLog:@"ready for etc signature"];
        iRtn = PAEW_DeriveTradeAddress(ppPAEWContext, devIdx, nCoinType, puiDerivePathETC, sizeof(puiDerivePathETC)/sizeof(puiDerivePathETC[0]));
        if (iRtn != PAEW_RET_SUCCESS) {
            [self printLog:@"etc signature failed due to PAEW_DeriveTradeAddress returns : %@", [Utils errorCodeToString:iRtn]];
            return ;
        }
        
        unsigned char transaction[] = { 0xEC, 0x0C, 0x85, 0x04, 0xA8, 0x17, 0xC8, 0x00, 0x83, 0x01, 0xD4, 0xC0, 0x94, 0xF2, 0xE6, 0xC2, 0xC9, 0xBD, 0xB2, 0xC2, 0x18, 0x3B, 0x04, 0x4E, 0x54, 0xE1, 0x76, 0xC7, 0xD5, 0xDB, 0x7C, 0x70, 0xFE, 0x87, 0x03, 0x8D, 0x7E, 0xA4, 0xC6, 0x80, 0x00, 0x80, 0x3D, 0x80, 0x80 };
        unsigned char pbTXSig[1024] = {0};
        size_t pnTXSigLen = 1024;
        BOOL pinVerified = NO;
        unsigned char authType = PAEW_SIGN_AUTH_TYPE_FP;
        
        iRtn = PAEW_ETC_SetTX(ppPAEWContext, devIdx, transaction, sizeof(transaction));
        if (iRtn != PAEW_RET_SUCCESS) {
            [self printLog:@"eth signature failed due to PAEW_ETC_SetTX returns :  %@", [Utils errorCodeToString:iRtn]];
            return;
        }
        ...
        while (YES) {
            ...
            //after all, loop to get sign result
            iRtn = PAEW_ETC_GetSignResult(ppPAEWContext, devIdx, authType, pbTXSig, &pnTXSigLen);
            
            if (iRtn == PAEW_RET_SUCCESS) {
                [self printLog:@"etc signature succeeded with signature: %@", [Utils bytesToHexString:pbTXSig length:pnTXSigLen]];
                needAbort = false;
                break;
            } else if (lastResult != iRtn) {
                [self printLog:@"%@ signature status : %@", (authType == PAEW_SIGN_AUTH_TYPE_FP ? @"Fingerprint" : @"PIN"), [Utils errorCodeToString:iRtn]];
...
- (void)CYBSignBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        int iRtn = PAEW_RET_UNKNOWN_FAIL;
        unsigned char nCoinType = PAEW_COIN_TYPE_CYB;
        iRtn = PAEW_DeriveTradeAddress(ppPAEWContext, devIdx, nCoinType, puiDerivePathCYB, sizeof(puiDerivePathCYB)/sizeof(puiDerivePathCYB[0]));
        if (iRtn != PAEW_RET_SUCCESS) {
...
- (void)EOSSignBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        int iRtn = PAEW_RET_UNKNOWN_FAIL;
        unsigned char nCoinType = PAEW_COIN_TYPE_EOS;
        iRtn = PAEW_DeriveTradeAddress(ppPAEWContext, devIdx, nCoinType, puiDerivePathEOS, sizeof(puiDerivePathEOS)/sizeof(puiDerivePathEOS[0]));
        if (iRtn != PAEW_RET_SUCCESS) {
...
- (void)getFWVersionBtnAction
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (initState == PAEW_RET_SUCCESS) {
            NSString *algVer = [NSString stringWithUTF8String:version.pbAlgVersion];
            ...
            NSString *bleVer = [Utils bytesToHexString:version.pbBLEVersion length:sizeof(version.pbBLEVersion)];
...
- (void)getDevInfoBtnAction
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        nDevInfoType = PAEW_DEV_INFOTYPE_COS_TYPE | PAEW_DEV_INFOTYPE_COS_VERSION | PAEW_DEV_INFOTYPE_SN | PAEW_DEV_INFOTYPE_CHAIN_TYPE | PAEW_DEV_INFOTYPE_PIN_STATE | PAEW_DEV_INFOTYPE_LIFECYCLE | PAEW_DEV_INFOTYPE_BLE_VERSION;
        ...
        if (devInfoState == PAEW_RET_SUCCESS) {
            ...
            [self printLog:@"COS Version: %@", [Utils bytesToHexString:devInfo.pbCOSVersion length:sizeof(devInfo.pbCOSVersion)]];
            [self printLog:@"BLE Version: %@", [Utils bytesToHexString:devInfo.pbBLEVersion length:sizeof(devInfo.pbBLEVersion)]];
